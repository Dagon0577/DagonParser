...
public class MySQLDMLHandlerParserTest extends AbstractSyntaxTest {
    public void testHandler() throws Exception {
        ...
        TestCase.assertEquals("HANDLER tbl_name OPEN", output);
        ...
        TestCase.assertEquals("HANDLER tbl_name OPEN AS handler1", output);

        sql = "HANDLER tbl_name READ index_name  = (t0,t1) where id = 1; ".getBytes();
        tuple = Parser.parse(sql, MySqlCharset.UTF8_FOR_JAVA);
        handler = (DMLHandlerStatement) tuple._1().get(0);
        output = outputMySQL(handler, sql);
        TestCase.assertEquals("HANDLER tbl_name READ index_name=(t0,t1) WHERE id=1", output);

        sql = "HANDLER tbl_name READ index_name  <= (t1) where id = 1 LIMIT 10;".getBytes();
        tuple = Parser.parse(sql, MySqlCharset.UTF8_FOR_JAVA);
        handler = (DMLHandlerStatement) tuple._1().get(0);
        output = outputMySQL(handler, sql);
        TestCase.assertEquals("HANDLER tbl_name READ index_name<=(t1) WHERE id=1 LIMIT 10", output);
        ...
        handler = (DMLHandlerStatement) tuple._1().get(0);
        output = outputMySQL(handler, sql);
        TestCase.assertEquals("HANDLER tbl_name READ index_name<(t0,t1) WHERE id=1 LIMIT 10", output);
        ...
        handler = (DMLHandlerStatement) tuple._1().get(0);
        output = outputMySQL(handler, sql);
        TestCase.assertEquals("HANDLER tbl_name READ index_name FIRST", output);

        sql = "HANDLER tbl_name READ index_name NEXT;".getBytes();
        tuple = Parser.parse(sql, MySqlCharset.UTF8_FOR_JAVA);
        handler = (DMLHandlerStatement) tuple._1().get(0);
        output = outputMySQL(handler, sql);
        TestCase.assertEquals("HANDLER tbl_name READ index_name NEXT", output);

        sql = "HANDLER tbl_name READ index_name PREV;".getBytes();
        tuple = Parser.parse(sql, MySqlCharset.UTF8_FOR_JAVA);
        handler = (DMLHandlerStatement) tuple._1().get(0);
        output = outputMySQL(handler, sql);
        TestCase.assertEquals("HANDLER tbl_name READ index_name PREV", output);

        sql = "HANDLER tbl_name READ index_name LAST WHERE id=1 LIMIT 10;".getBytes();
        tuple = Parser.parse(sql, MySqlCharset.UTF8_FOR_JAVA);
        handler = (DMLHandlerStatement) tuple._1().get(0);
        output = outputMySQL(handler, sql);
        TestCase.assertEquals("HANDLER tbl_name READ index_name LAST WHERE id=1 LIMIT 10", output);

        sql = "HANDLER tbl_name READ FIRST;".getBytes();
        tuple = Parser.parse(sql, MySqlCharset.UTF8_FOR_JAVA);
        handler = (DMLHandlerStatement) tuple._1().get(0);
        output = outputMySQL(handler, sql);
        TestCase.assertEquals("HANDLER tbl_name READ FIRST", output);

        sql = "HANDLER tbl_name READ NEXT WHERE id=1 LIMIT 9;".getBytes();
        tuple = Parser.parse(sql, MySqlCharset.UTF8_FOR_JAVA);
        handler = (DMLHandlerStatement) tuple._1().get(0);
        output = outputMySQL(handler, sql);
        TestCase.assertEquals("HANDLER tbl_name READ NEXT WHERE id=1 LIMIT 9", output);

        sql = "HANDLER tbl_name CLOSE;".getBytes();
        tuple = Parser.parse(sql, MySqlCharset.UTF8_FOR_JAVA);
        handler = (DMLHandlerStatement) tuple._1().get(0);
        output = outputMySQL(handler, sql);
        TestCase.assertEquals("HANDLER tbl_name CLOSE", output);
    }
}
